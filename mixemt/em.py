...
def em_step(read_hap_mat, weights, ln_props, read_mix_mat):
    ...
    numpy.subtract(read_mix_mat,
                   scipy.special.logsumexp(read_mix_mat, axis=1).reshape((-1, 1)),
                   read_mix_mat)

    # M-Step:
    # Set theta_g - contribution of g to the mixture
    new_props = scipy.special.logsumexp(read_mix_mat, axis=0,
                                     b=weights.reshape((-1, 1)))
    new_props -= scipy.special.logsumexp(new_props)
